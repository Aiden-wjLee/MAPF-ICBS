...
def compute_heuristics(my_map, goal):
    ...
    for loc, node in closed_list.items():
        h_values[loc] = node["cost"]  # + random.random()  # loc : (y, x) format
    # usage
    # h_values[3, 4] = 0 # (y=3, x=4) heursitcis is 0

    # custom
    h_values = np.zeros((len(my_map), len(my_map[0])))
    for loc, node in closed_list.items():
...
class A_Star(object):
    ...
    def push_node(self, node):
        ...
        heapq.heappush(
            self.open_list,
            (f_value, node["h_val"], node["loc"], self.num_generated, node),
        )
            ...
            if agent == constraint["agent"]:
                ...
                if len(constraint["loc"]) == 1:
                    ...
                    # negative constraint
                        ...
                        not constraint["positive"] and next_loc == constraint["loc"][0]
                    ):
                        # print("time {} negative constraint : {}".format(timestep, constraint))
                        return constraint
                # edge constraint
                else:
                    if constraint["positive"] and constraint["loc"] != [
                        curr_loc,
                        next_loc,
                    ]:
                        # print("time {} positive constraint : {}".format(timestep, constraint))
                        return constraint
                    if not constraint["positive"] and constraint["loc"] == [
                        curr_loc,
                        next_loc,
                    ]:
                        # print("time {} negative constraint : {}".format(timestep, constraint))
    ...
    def future_constraint_violated(
        self, curr_loc, timestep, max_timestep, c_table_agent, agent
    ):
    ...
    def generate_child_nodes(self, curr):
        ...
        children = []
        ma_dirs = product(
            list(range(5)), repeat=len(self.agents)
        ...
        for dirs in ma_dirs:
            ...
            for i, a in enumerate(self.agents):
                ...
                for j, a in enumerate(self.agents):
                    if i != j:
                        # print(ai, aj)
                        if (
                            child_loc[i] == curr["loc"][j]
                            and child_loc[j] == curr["loc"][i]
                        ):
                            invalid_move = True
            ...
            for i, a in enumerate(self.agents):
                ...
                if (
                    next_loc[0] < 0
                    or next_loc[0] >= len(self.my_map)
                    or next_loc[1] < 0
                    or next_loc[1] >= len(self.my_map[0])
                ):
                    invalid_move = True
            ...
            h_test = sum(
                [self.heuristics[i][child_loc[i]] for i in range(len(self.agents))]
            )
            ...
            for i, a in enumerate(self.agents):
                ...
                if not reached_goal[i] and child_loc[i] == self.goals[i]:
                    ...
                    if curr["timestep"] + 1 <= self.max_constraints[i]:
                        if not self.future_constraint_violated(
                            child_loc[i],
                            curr["timestep"] + 1,
                            self.max_constraints[i],
                            self.c_table[i],
                            self.agents[i],
                        ):
    ...
    def find_paths(self):
        ...
        h_value = sum(
            [
                self.heuristics[i][self.starts[i]] for i in range(len(self.agents))
            ]  # self.starts[i] : (y, x) format
        )

        # assert h_value == h_test

        root = {
            "loc": [self.starts[j] for j in range(len(self.agents))],
        ...
        while len(self.open_list) > 0:
            ...
            solution_found = all(
                curr["reached_goal"][i] for i in range(len(self.agents))
            )
            ...
            for child in children:
                ...
                if (tuple(child["loc"]), child["timestep"]) in self.closed_list:
                    existing = self.closed_list[
                        (tuple(child["loc"]), child["timestep"])
                    ]
                    if (
                        (
                            child["g_val"] + child["h_val"]
                            < existing["g_val"] + existing["h_val"]
                        )
                        and (child["g_val"] < existing["g_val"])
                        and child["reached_goal"].count(False)
                        <= existing["reached_goal"].count(False)
                    ):
                        print("child is better than existing in closed list")
                        self.closed_list[(tuple(child["loc"]), child["timestep"])] = (
                            child
                        )
                        self.push_node(child)
                else:
